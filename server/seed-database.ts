...
async function generateSyntheticData(): Promise<Item[]> {
    ...
    Generate 10 furniture store items. Each record should include the following fields: 
    item_id, item_name, item_description, brand, manufacturer_address, prices, categories,
    user_reviews, notes. Ensure variety in the data and realistic values.

    ${parser.getFormatInstructions()}`  // Add format instructions from parser

    console.log('Generating syncthetic data ...')
